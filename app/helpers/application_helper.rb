module ApplicationHelper
  def profile_img(user)
    return image_tag(user.avatar, alt: user.name, width: 48, hight: 48, class: "img-rounded") if user.avatar?
    ...
    unless user.provider.blank?
      ...
      image_tag(img_url, alt: user.name, width: 48, hight: 48, class: "img-rounded")
  end
